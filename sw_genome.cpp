...
int SmithWaterman(const string& ref, const string& pattern, int& bestPos) {
    ...
    // (m+1) x (n+1) DP table, initialized to 0
    ...
    // estimate start position: end position minus number of matched rows
...
string reconstruct(const string& ref, const vector<string>& reads) {
    ...
    int interval = max(1, total / 10); // log every 10%
...
int main(int argc, char* argv[]) {
    ...
    cout << "Number of SNPs:   " << snps.size() << endl << endl;
    ...
    ofstream out("restored_genome.txt");
    if (!out) { cerr << "Failed to save restored_genome.txt" << endl; return 1; }
    out << reconstructed;
    out.close();
    cout << "Restored genome saved to restored_genome.txt" << endl;
    ...
    ofstream log("sw_log.txt", ios::app);
    ...
    log << endl;
    log.close();
    cout << "Log saved to sw_log.txt" << endl;
